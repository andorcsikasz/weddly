...
- **Founding (free) is automatic for the first `FOUNDING_CAP`=200 created couples — no partner required.** `initBillingAtOnboarding` (called at couple creation in `routes/couples.ts`) checks `isFoundingEligible` (creation-order rank `< 200`, by `couples.id`, demo couples excluded) and, if eligible, immediately sets `subscription_status='founding'`, `is_founding_member=1`, `founding_until = now + 18 months`. Couples past the cap get the trial instead, and separately every partnered couple (cap or not) gets a "free until your wedding day" founding window when partner B joins (`activatePartnerFreeWindow`) — but the `is_founding_member` badge stays reserved for the first 200. Covered by `backend/tests/api/billing.e2e.test.ts`.
