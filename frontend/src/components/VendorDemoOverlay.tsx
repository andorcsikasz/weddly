// Vendor-side demo banner + conversion nudge. Lives inside /vendor whenever
// the active session is the Shrek-themed bakery demo.
//
// Detection is authoritative and network-free: the demo vendor's email always
...
// of a stale localStorage flag mislabelling a real vendor.
//
// Two surfaces, mirroring PlannerDemoOverlay:
//   1. A top bar, demo label · changes vanish · start your own. Placed above
//      the sticky header in normal flow so it scrolls away instead of fighting
//      the header for top:0.
//   2. A nudge popup after ~3 minutes → /vendors/signup (start your own).
...
const NUDGE_LAST_SEEN_KEY = "weddly.vendor_demo_nudge_last_seen";
...
export function VendorDemoOverlay() {
  const { t } = useT();
  ...
  const isDemo = user?.role === "vendor" && user.email.toLowerCase().endsWith(DEMO_EMAIL_SUFFIX);
        ...
        >
          <div className="mx-auto flex max-w-7xl items-center gap-3 sm:gap-4">
            ...
            <button
              ...
              aria-label={t("vendor_demo.banner_dismiss_aria")}
...
function VendorDemoNudgeModal({
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="vendor-demo-nudge-title"
